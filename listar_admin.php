...
try {
    if ($busca && trim($busca) !== '') {
        $stmt = $pdo->prepare("SELECT * FROM ADMINISTRADOR WHERE ADM_NOME LIKE ? AND ADM_ATIVO = ?");
        $stmt->execute(array("%$busca%", $status));
    } else if ($status !== null) {
        $stmt = $pdo->prepare("SELECT * FROM ADMINISTRADOR WHERE ADM_ATIVO = ?");
...
if (isset($_SESSION['success_message_admin'])) {
    ...
    <span class='mr-2'>" . $_SESSION['success_message_admin'] . "</span> 
    </div>";
        ...
        <section class="w-full bg-[#00AFEF] rounded-tl-lg pt-6 pb-4 px-10">
            <div class="flex items-center justify-between">
                <div class="flex gap-4">
                    <img src="icons/admin.svg">
                    <h2 class="text-white font-bold text-3xl">Administradores</h2>
                </div>
                <a href="cadastrar_admin.php" class="text-white">
            ...
            <div class="mt-4">
                <form class="flex items-center justify-between" action="listar_admin.php" method="POST" id="searchForm">

                    <div class="flex items-center">
                        <input class="p-2 rounded-l-lg outlined-none" type="text" name="busca" placeholder="Buscar por nome do admin" value="<?php echo htmlspecialchars($busca); ?>" id="searchInput">
            ...
            <div>
                    ...
                    <table>
                        ...
                        <?php foreach ($adms as $adm) :  ?>
                            <tr>
                                ...
                                <td>
                                    <div class="rounded-lg m-auto text-white w-20" style="background-color: <?php echo $adm['ADM_ATIVO'] ? '#109010' : '#D90000'; ?>;">
                                ...
                                <td >
                                    ...
                                    <a class="mr-2" href="editar_admin.php?id=<?php echo $adm['ADM_ID']; ?>">
                                        <button>
                                            <img src="icons/edit.svg" />
                                        </button>
                                    </a>
                                    <button onclick="openConfirmDelete(<?php echo $adm['ADM_ID']; ?>)" id="delete">
                ...
                <div class="flex flex-col items-center mt-16">
                    <h3 class="font-bold text-white text-lg">Nenhum administrador encontrado</h3>
                    <img src="imagens/search.png" class="w-[500px]">
                </div>
                <?php endif; ?>
            </div>
        </section>
            ...
            <dialog id="confirmDelete" class="bg-white border-2 border-[#0b7ca5] drop-shadow-lg w-fit font-bold p-6 flex flex-col justify-between gap-6 rounded-lg">
                <form action="excluir_admin.php" method="get">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        <h3 class="font-extrabold text-[#0b7ca5]">Tem certeza que deseja excluir este administrador?</h3>
                    </div>
