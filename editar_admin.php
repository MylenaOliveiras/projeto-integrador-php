...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $_SESSION['success_message_admin'] = "Administrador atualizado com sucesso!";
        ...
        <span class='mr-2'>Erro ao editar o administrador</span> 
        " . $e->getMessage() . "
        </div>";    
    }
}
...
<body>
    ...
    <h2 class="text-white font-bold text-3xl">Editar administrador</h2>

    <div class="w-[612px] mt-10">
      <form class="flex flex-col gap-6" action="editar_admin.php" method="post">
    ...
    <!-- Essa linha cria um campo de entrada (input) oculto no formulário. Um campo de entrada oculto é usado quando você quer incluir um dado no formulário que não precisa ser visível ou editável pelo usuário, mas que precisa ser enviado junto com os outros dados quando o formulário é submetido. -->
        <div class="flex">
    <input type="text" name="nome" id="nome" value="<?php echo $adm['ADM_NOME']; ?>">
     <div class="icon">
            <img src="icons/user.svg">
        </div>
    </div>
        <div class="flex">
    <input type="email" name="email" id="email" value="<?php echo $adm['ADM_EMAIL']; ?>">
     <div class="icon">
            <img src="icons/user.svg">
        </div>
    </div>
        <div class="flex">
    <input type="password" name="senha" id="senha" value="<?php echo $adm['ADM_SENHA']; ?>">
     <div class="icon">
            <img src="icons/user.svg">
        </div>
    </div>
     ...
     <div class="flex items-center justify-between">
            ...
            <label class="switch">
            <input type="checkbox" name="ativo" id="ativo" <?php echo ($adm['ADM_ATIVO'] == 1) ? 'checked' : ''; ?>  <?php echo $switchDisabled ?>>
            <span class="slider round"></span>
            </label>
        </div>
    
    </div>
    <input type="submit" value="Atualizar Administrador" class="bg-[#0b7ca5] px-3 py-2 text-white rounded-md">
